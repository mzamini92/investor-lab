...
def _weight_map(holdings: ETFHoldings) -> dict[str, float]:
    return {holding.stock_ticker: holding.weight for holding in holdings.holdings}


def compute_pair_overlap(
    etf_a: str,
    etf_b: str,
    holdings_a: ETFHoldings,
    holdings_b: ETFHoldings,
    portfolio_weight_a: float,
    portfolio_weight_b: float,
    ...
    min_count = min(len(tickers_a), len(tickers_b))
    unweighted_overlap = shared_count / min_count if min_count else 0.0
    practical_overlap = sum(
        min(portfolio_weight_a * weights_a[ticker], portfolio_weight_b * weights_b[ticker]) for ticker in intersection
    )
    ...
    return {
        ...
        "unweighted_overlap": round(unweighted_overlap, 6),
        "weighted_overlap": round(weighted_overlap, 6),
        "jaccard_overlap": round(jaccard_overlap, 6),
        "practical_overlap_contribution": round(practical_overlap, 6),
    }
...
def build_overlap_matrix(
    ...
    Dict[str, Dict[str, Dict[str, Union[float, int, str]]]],
    List[Dict[str, Union[float, int, str]]],
]:
    tickers = sorted(holdings_map)
    matrix: Dict[str, Dict[str, Dict[str, Union[float, int, str]]]] = {ticker: {} for ticker in tickers}
    pair_rows: List[Dict[str, Union[float, int, str]]] = []
    ...
    for ticker in tickers:
        ...
        matrix[ticker][ticker] = {
            ...
            "jaccard_overlap": 1.0,
            "practical_overlap_contribution": round(portfolio_weights[ticker], 6),
        }

    for etf_a, etf_b in combinations(tickers, 2):
        metrics = compute_pair_overlap(
            etf_a=etf_a,
            etf_b=etf_b,
            holdings_a=holdings_map[etf_a],
            holdings_b=holdings_map[etf_b],
            portfolio_weight_a=portfolio_weights[etf_a],
            portfolio_weight_b=portfolio_weights[etf_b],
        )
        pair_rows.append(metrics)
        matrix[etf_a][etf_b] = metrics
        matrix[etf_b][etf_a] = {**metrics, "etf_a": etf_b, "etf_b": etf_a}

    return matrix, sorted(pair_rows, key=lambda row: row["weighted_overlap"], reverse=True)
