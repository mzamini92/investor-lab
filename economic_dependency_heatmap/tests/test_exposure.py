...
def build_provider() -> MockDependencyDataProvider:
            ...
            "ETF1": ETFHoldings(
                ...
                holdings=[
                    HoldingRecord(
                        ...
                        us_consumer=0.4, china_demand=0.2, europe_demand=0.2, ai_capex=0.8, cloud_spending=0.7, global_semiconductors=0.9, energy_prices=0.0, industrial_capex=0.1, emerging_market_growth=0.2, healthcare_spending=0.0, financial_conditions=0.1, usd_strength=0.4, interest_rate_sensitivity=0.3,
                    ),
                    HoldingRecord(
                        ...
                        us_consumer=0.2, china_demand=0.1, europe_demand=0.1, ai_capex=0.0, cloud_spending=0.0, global_semiconductors=0.0, energy_prices=0.0, industrial_capex=0.0, emerging_market_growth=0.1, healthcare_spending=0.8, financial_conditions=0.1, usd_strength=0.2, interest_rate_sensitivity=0.2,
                    ),
                ],
            ),
            "ETF2": ETFHoldings(
                ...
                holdings=[
                    ...
                    HoldingRecord(
                        ...
                        us_consumer=0.1, china_demand=0.2, europe_demand=0.5, ai_capex=0.1, cloud_spending=0.0, global_semiconductors=0.0, energy_prices=0.1, industrial_capex=0.7, emerging_market_growth=0.2, healthcare_spending=0.0, financial_conditions=0.1, usd_strength=0.5, interest_rate_sensitivity=0.3,
                    ),
                ],
            ),
        },
        scenarios=[MacroScenario(name="ai_boom", display_name="AI Boom", description="AI spending rises.", shock_weights={"ai_capex": 1.0, "cloud_spending": 0.5, "global_semiconductors": 0.5})],
    )


def test_underlying_exposure_aggregation() -> None:
    analyzer = EconomicDependencyAnalyzer(build_provider())
...
def test_revenue_geography_aggregation() -> None:
    analyzer = EconomicDependencyAnalyzer(build_provider())
    result = analyzer.analyze([{"ticker": "ETF1", "amount": 50}, {"ticker": "ETF2", "amount": 50}])
    revenue_map = {row["name"]: row["exposure"] for row in result.revenue_geography_exposure_table}
